...
import { CREATE_ROLE, EDIT_ROLE, GET_ALL_ROLES } from "../apiURls";
...
export const editRole = async ({ payload, headers }: any) => {
  try {
    const response = await axiosInstance.post(EDIT_ROLE, payload, {
      headers,
    });
    return { result: response.data };
  } catch (error: any) {
